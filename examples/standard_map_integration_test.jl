...
alg_kwargs = ();
FT = Float64
model = StandardMap{Float64}(4.0)
@everywhere evolve_wrapper(tspan) = (u0) -> evolve_mapping_system(model, u0, tspan)
...
tspan_direct = (0.0, algorithm_cost)
direct_solution = evolve_mapping_system(model, u0[end-1], tspan_direct)
